			...
			public void actionPerformed(ActionEvent e) {
				if(!fila.vazia()) {
					...
					try {
						...
						JOptionPane.showMessageDialog(null, "Elemento removido: " + removido, "Removido",		JOptionPane.INFORMATION_MESSAGE);
					}
					catch(Exception e2) {
		...
		botaoFilaBuscar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(!fila.vazia()) {
					...
					inicio = (inicio + 1) % 10;
					fila.primeiro();
					JOptionPane.showMessageDialog(null, "Primeiro elemento da fila: " + buscar, "Busca", JOptionPane.INFORMATION_MESSAGE);
				}
				else {
					JOptionPane.showMessageDialog(null, "Fila vazia!", "Erro", JOptionPane.ERROR_MESSAGE);
				}
